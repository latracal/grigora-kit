...
if(!function_exists("render_block_grigora_kit_post_grid_2")){
	function render_block_grigora_kit_post_grid_2( $attributes, $content, $block ) {
		...
		if(count($data) == 5) {
			...
			$first_block = sprintf('<%1$s class="big-style">%2$s%3$s%4$s%5$s%6$s%7$s%8$s%9$s</%10$s>',
			...
			$fifth_block = sprintf('<%1$s class="block2345 small-style">%2$s%3$s%4$s%5$s%6$s%7$s%8$s</%9$s>',
				...
				(
					($categoryToggle ? 
						(
							( '<div class="cat-container"> <a class="category-style" target="' ). (isset( $attributes["newTab"] ) ? ( $attributes["newTab"] ? "_blank" : "_self" ) : "_self"). 
							( '" href="' ) . ($categoryLink ? get_category_link(get_the_category($data[4]->ID)[0]->term_id) : get_permalink($data[4]->ID)) . ( '">' ) . (get_the_category($data[4]->ID)[0]->name) . ('</a> </div>')
						) 
						: ' '
					) .
					sprintf('<%1$s class="meta-styletitle-container titleS-style">%2$s</%3$s>',
