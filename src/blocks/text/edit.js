...
import { ENTRANCE_ANIMATIONS, TEXT_TRANSFORMS, TEXT_STYLE, TEXT_DECORATION, FONT_WEIGHTS } from '@constants';
		...
		return (
			<>
				<GrigoraColorGradientInput
					...
					onColorChange={ (textColor) => setAttributes({ textColor }) }
					onGradientChange={ (textGradient) => setAttributes({ textGradient }) }
		...
		return(
			<div className={`grigora-hover-effects-panel`}>
				<GrigoraColorGradientInput
					...
					onColorChange={ (textHColor) => setAttributes({ textHColor }) }
					onGradientChange={ (textHGradient) => setAttributes({ textHGradient }) }
					label={ __( 'Text', "grigora-kit" ) }
				/>
				{/* <GrigoraColorGradientInput
				...
				unit={"sec"}
				step={0.1}
				setValue={ transitionColorTime => setAttributes( { transitionColorTime } ) }
		...
		return(
			<>
				<GrigoraColorInput
					...
					onChange={ textShadowHColor => setAttributes( { textShadowHColor } ) }
					resetValue={'#000'}
				/>
				<HStack spacing={ 2 }>
					<GrigoraUnitInput
						label="Blur"
						onChange={ textShadowHBlur => setAttributes( { textShadowHBlur } ) }
						value={textShadowHBlur}
						resetValue={ "0px" }
					/>
					<GrigoraUnitInput
						label="Horizontal"
						onChange={ textShadowHHorizontal => setAttributes( { textShadowHHorizontal } ) }
						value={textShadowHHorizontal}
						resetValue={ "0px" }
					/>
					<GrigoraUnitInput
						label="Vertical"
						onChange={ textShadowHVertical => setAttributes( { textShadowHVertical } ) }
		...
		<div { ...blockProps }>
			...
			<InspectorControls>
				<PanelBody title={ __( 'Typography', "grigora-kit" ) } initialOpen={false}>
					<GrigoraRangeInput
						value={typoSize} 
						setValue={(typoSize)=>{setAttributes({typoSize});}} 
						label={`Size`} 
						resetValue={16} 
					/>
					<GrigoraRangeInput 
						value={typoLineHeight} 
						setValue={(typoLineHeight)=>{setAttributes({typoLineHeight:typoLineHeight.toString()})}} 
						label={`Line Height`} 
						min={10} 
						max={300} 
						resetValue={"normal"} 
					...
					<GrigoraRangeInput 
						...
						setValue={(typoLetterSpacing)=>{setAttributes({typoLetterSpacing:typoLetterSpacing.toString()})}} 
					...
					<GrigoraRangeInput 
						...
						setValue={(typoWordSpacing)=>{setAttributes({typoWordSpacing:typoWordSpacing.toString()})}} 
						label={`Word Spacing`} 
						min={0} 
						max={150} 
						resetValue={"normal"} 
				...
				<GrigoraRangeInput 
					value={layoutColumns} 
					setValue={(layoutColumns)=>{setAttributes({layoutColumns:layoutColumns.toString()})}} 
					label={`Columns`} 
					min={1} 
					max={10} 
					resetValue={"default"} 
				...
				<GrigoraRangeInput 
					...
					setValue={(layoutColumnsGap)=>{setAttributes({layoutColumnsGap:layoutColumnsGap.toString()})}} 
					label={`Columns Gap`} 
					min={1} 
					max={100} 
					resetValue={"normal"} 
			...
			<style>
					{` .block-id-${id} {
						...
						column-count: ${
							layoutColumns ? layoutColumns : "default"
						};
						column-gap: ${
							layoutColumnsGap ? `${layoutColumnsGap}px` : "normal"
						};
					...
					${ textHColor ? `.block-id-${id}:hover {${textColor ? `color`: `-webkit-text-fill-color`}: ${textHColor};} ` : `` }
					${ textHGradient ? `.block-id-${id} {background-image: ${textHGradient};-webkit-background-clip: text;} .block-id-${id}:hover {color: transparent;} ` : `` }
					${ ((textShadowHHorizontal && textShadowHHorizontal!="0px") || (textShadowHVertical && textShadowHVertical!="0px") || (textShadowHBlur && textShadowHBlur!="0px")) ? `.block-id-${id}:hover{filter: drop-shadow(${`${ textShadowHHorizontal ? textShadowHHorizontal : "0px" } ${ textShadowHVertical ? textShadowHVertical : "0px" } ${ textShadowHBlur ? textShadowHBlur : "0px" } ${ textShadowHColor ? textShadowHColor: "#000" }`});}` : `` }
					`}
			</style>
			<RichText
