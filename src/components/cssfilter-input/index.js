...
function GrigoraCSSFilterInput( { value, setValue, label = "", resetValue={} } ){

	const [ openPopOver, setOpenPopOver ] = useState( false );

	const [ blur, setBlur ] = useState( value.blur ? value.blur : 5 );
	const [ brightness, setBrightness ] = useState( value.brightness ? value.brightness : 100 );
	const [ contrast, setContrast ] = useState( value.contrast ? value.contrast : 100 );
	const [ saturation, setSaturation ] = useState( value.saturation ? value.saturation : 135 );
