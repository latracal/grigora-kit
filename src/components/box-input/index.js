	...
	return (
		<div className={ `grigora-box-input` }>
			<HStack spacing={ 4 }>
				...
				{ (! deepEqualObj( values, resetValue ) || ! deepEqualObj( valueTablet, resetValueTablet ) ||! deepEqualObj( valueMobile, resetValueMobile ) ) && (
					<GrigoraResetButton
						onClick={ () => {
							onChange( resetValue );
							if(isResponsive){
			...
			<div className="grigora-box-input__select">
				<BoxControl
					...
					values={ activeResponse==='desktop' ? values : (activeResponse==='tablet' ? valueTablet : valueMobile) }
				/>
			</div>
		</div>
	);
