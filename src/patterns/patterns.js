...
function GrigoraPatterns( props ){
    ...
    const [ categories, setCategories ] = useState([{name: 'All', count: 0, active: true }]);
        ...
        for (var i = 0; i < arrayLength; i++) {
            ...
            if( data[i].tags ){
                ...
                for (var j = 0; j < arrayLength2; j++) {
                    if(data[i]){
                        temp_cat[data[i].tags[j]] = (temp_cat[data[i].tags[j]]+1) || 1 ;
                    }
                }
            }
        }
    ...
    useEffect( () => {
        ...
        if( cache ){
            ...
            .then((data) => {
                if(data.success){
                    set_cache( "grigora_get_patterns_meta_data", data.data );
    ...
    const patterns_filtered =  Object.values(patterns).filter(function (el) {
        if( category === "All" ){
            return true;
        }
        else{
                        ...
                        {patterns_filtered.map((e) =>
                                ...
                                >
                                    ...
                                    <div className='overlay'>
                                        <div className='import' onClick={(element)=>{
                                            if(!importing){
                                                setImporting(true);
                                                element.target.closest('.select-pattern').classList.add('active');
                                                importPattern(e.id);
                                            }
                                        }}>{__('Import', 'grigora-kit')}</div>
                                        <a className='preview' href={e.url} target="_blank">{__('Preview', 'grigora-kit')}</a>
