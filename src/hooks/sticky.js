...
export const withInspectorControl = createHigherOrderComponent(
	( BlockEdit ) => {
		return ( props ) => {
			...
			const { sticky, sticky_data } = attributes;
			...
			if ( hasSupport && props.isSelected ) {
				return (
					<>
						...
						<InspectorControls>
							<PanelBody
								title={ __( 'Sticky', 'grigora-kit' ) }
								className={ 'sticky_panel' }
								initialOpen={ false }
							>
								<SelectControl
								...
								{ sticky !== 'none' && (
									<>
										...
										<StickyControl
											value={ sticky_data }
											onChange={ ( sticky_data ) =>
												setAttributes( { sticky_data } )
											}
										/>
									</>
								) }
...
export function addSaveProps( extraProps, blockType, attributes ) {
	if (
		...
		if ( ! isEmpty( attributes.sticky_data ) ) {
			...
			extraProps[ 'data-sticky_data' ] = JSON.stringify( {
				...attr.sticky_data,
...
addFilter(
	...
	'grigora-kit/grigoraSticky/attribute',
	addAttribute
);
addFilter(
	'editor.BlockEdit',
