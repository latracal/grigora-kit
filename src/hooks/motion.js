					...
					<>
						...
						<InspectorControls>
							<PanelBody
								...
								className={ 'motion_effects_panel' }
								initialOpen={ false }
							>
								<ToggleControl
									label={ __(
									...
									onChange={ () => {
										if ( !! motionanimation_mouse ) {
											if (
												typeof motion_animation_stop_listeners ===
												'function'
											) {
											...
											if (
												typeof motion_animation_restart_listeners ===
												'function'
											) {
...
export function addSaveProps( extraProps, blockType, attributes ) {
	if (
		...
		( attributes.motionanimation_mouse ||
			attributes.motionanimation_scroll )
	) {
		var data_export = {};
		...
		if ( attributes.motionanimation_mouse ) {
			...
			if ( ! isEmpty( data_export ) ) {
				extraProps[ 'data-motionanimation_mouse' ] =
					JSON.stringify( data_export );
		...
		if ( attributes.motionanimation_scroll ) {
			...
			data_export = clearEmpties(
				attributes.motionanimation_scroll_data
			);
			if ( ! isEmpty( data_export ) ) {
				extraProps[ 'data-motionanimation_scroll' ] =
					JSON.stringify( data_export );
				extraProps.className = classnames(
					extraProps.className,
					'has-motion-animations'
...
export function addEditProps( settings ) {
	if ( hasBlockSupport( settings, 'grigoraMotion', false ) ) {
...
addFilter(
	...
	'grigora-kit/grigoraMotion/attribute',
	addAttribute
);
addFilter(
	'editor.BlockEdit',
